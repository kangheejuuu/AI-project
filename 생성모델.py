...
# 저장할 경로 설정
output_folder="C:/yoloV5/yolov5/generated_images3/"

# 폴더가 존재하지 않으면 폴더를 생성합니다
os.makedirs(output_folder, exist_ok=True)

# 이미지 저장 함수 정의
def save_image_batch(image_batch, output_folder, epoch, i):
    for idx in range(image_batch.size(0)):
        image = image_batch[idx, :, :, :]
        vutils.save_image(image, f"{output_folder}/epoch_{epoch}_batch_{i}_image_{idx}.png")

# 이미지 저장
if (iters % 500 == 0) or ((epoch == num_epochs-1) and (i == len(dataloader)-1)):
    with torch.no_grad():
        fake = netG(fixed_noise).detach().cpu()
        save_image_batch(fake, output_folder, epoch, i)
