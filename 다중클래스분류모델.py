...
if __name__ == '__main__':
    ...
    class DisasterDetectionModel(nn.Module):
        def __init__(self):
            ...
            self.classifier = nn.Sequential(
                ...
                nn.ReLU(inplace=True),
                nn.Dropout(),
                nn.Linear(256, 6),  # earthquake fire flood landslide non-disaster typhoon 6가지
                #nn.Softmax(dim=1)   # 다중 클래스 분류에서는 softmax 사용
            )
    ...
    optimizer = optim.Adam(model.parameters(), lr=0.0001)  # 필요에 따라 조절
    scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=5, gamma=0.1) # 추가
    torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)


    # 학습
    num_epochs = 25  # 필요에 따라 조정
    ...
    # 학습된 모델 저장
    torch.save(model.state_dict(), 'Disastermodel.pth')
